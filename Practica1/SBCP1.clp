...
;; Regla para deducir que algo es un alimento
(defrule es_alimento
   (es_un_tipo_de ?x ?y)  ;; condición
   =>                    ;; acción
   (assert (es_alimento ?x))  ;; deducir que ?x es un alimento
)

;; Regla para imprimir los alimentos
(defrule imprime_alimentos
   (es_alimento ?x)  ;; condición
   =>              ;; acción
   (printout t crlf "Es un alimento: " ?x crlf)  ;; acción: imprimir
)
...
;;deduce (alimento_parecido ?x)  para los alimentos pertenezcan del mismo nivel que el alimento sobre el que se pregunta
(defrule alimento_mismo_nivel_piramide
   (alimento ?x)  ;; condición
   (es_un_tipo_de ?x ?y)  ;; condición
   (nivel_piramide_alimentaria ?y ?n)
   =>              ;; acción
   (assert (alimento_parecido ?x))  ;; deducir que ?x es un alimentossssssss
)

(defrule retracta_alimentos_parecidos ?f <- (alimento_parecido ?x)
   (propiedad ?p ?x ?v)  ;; condición
   (not (propiedad ?p ?x ?v))  ;; condición
   (neq ?v ?v)  ;; condición
   =>              ;; acción
   (retract ?f)  ;; deducir que ?x es un alimento
)

(defrule imprime_alimentos_parecidos
   (alimento_parecido ?x)  ;; condición
   =>              ;; acción
   (printout t crlf crlf "Es un alimento parecido: " ?x)  ;; acción: imprimir
)
