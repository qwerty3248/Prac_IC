...
;;Aqui empieza lo nuevo para los ninios 
(assert (modulo calcular_certeza_gustar_a_ninios))
)
)
;Aqui más cosas nuevas 
;Primero añadimos las funciones para trabajar con la certeza
;;Las Funcionees 
...
(deftemplate justificacion_certeza
(slot problema)
(slot contenido))
;Consideramos varios aspectos, como si tiene verdura, carne, es picante... que influiran positivamente o
;negativamente en la certeza.
(defrule tiene_verdura
(declare (salience 0))
(modulo calcular_certeza_gustar_a_ninios)
(propiedad_receta es_elegida ?receta)
(es_un_ingrediente_de ?i ?receta)
(es_un_tipo_de ?i verdura)
(not (Evidencia verdura si))
=>
(assert(Evidencia tiene_verdura si))
)
(defrule tiene_carne
(declare (salience 0))
(modulo calcular_certeza_gustar_a_ninios)
(propiedad_receta es_elegida ?receta)
(es_un_ingrediente_de ?i ?receta)
(es_un_tipo_de ?i carne)
(not (Evidencia tiene_carne si))
=>
(assert(Evidencia tiene_carne si))
)
(defrule tiene_dulce
(declare (salience 0))
(modulo calcular_certeza_gustar_a_ninios)
...
(not (Evidencia tiene_dulce si))
=>
(assert(Evidencia tiene_dulce si))
)
(defrule es_picante
(declare (salience 0))
(modulo calcular_certeza_gustar_a_ninios)
...
(not (Evidencia es_picante si))
=>
(assert(Evidencia es_picante si))
)
(defrule es_postre
(declare (salience 0))
(modulo calcular_certeza_gustar_a_ninios)
?receta <- (receta (nombre ?nombre) (tipo_plato $?tipos))
(test (member$ postre $?tipos))
(propiedad_receta es_elegida ?receta)
(not (Evidencia es_postre si))
=>
(assert(Evidencia es_postre si))
)
;Añadimos las reglas para modificar la certeza
(defrule R1
(declare (salience -1))
(modulo calcular_certeza_gustar_a_ninios)
(FactorCerteza tiene_verdura si ?f1)
(test (> ?f1 0))
=>
(assert (FactorCerteza ninios si (encadenado ?f1 -0.6)))
(assert (justificacion_certeza (problema ninios) (contenido "Tiene verdura, lo que hará que
le guste menos.")))
)
(defrule R2
(declare (salience -1))
(modulo calcular_certeza_gustar_a_ninios)
(FactorCerteza tiene_carne si ?f1)
(test (> ?f1 0))
=>
(assert (FactorCerteza ninios si (encadenado ?f1 0.3)))
(assert (justificacion_certeza (problema ninios) (contenido "Tiene carne, que le suele gustar
a los niños.")))
)
(defrule R3
(declare (salience -1))
(modulo calcular_certeza_gustar_a_ninios)
(FactorCerteza tiene_dulce si ?f1)
(test (> ?f1 0))
=>
(assert (FactorCerteza ninios si (encadenado ?f1 0.8)))
(assert (justificacion_certeza (problema ninios) (contenido "Tiene dulces, que le encantan a
los niños.")))
)
(defrule R4
(declare (salience -1))
(modulo calcular_certeza_gustar_a_ninios)
(FactorCerteza es_picante si ?f1)
(test (> ?f1 0))
=>
(assert (FactorCerteza ninios si (encadenado ?f1 -0.7)))
(assert (justificacion_certeza (problema ninios) (contenido "Tiene picante, que lo odian los
niños.")))
)
(defrule R5
(declare (salience -1))
(modulo calcular_certeza_gustar_a_ninios)
(FactorCerteza es_postre si ?f1)
(test (> ?f1 0))
=>
(assert (FactorCerteza ninios si (encadenado ?f1 0.9)))
(assert (justificacion_certeza (problema ninios) (contenido "Es un postre, que adoran los
niños.")))
)
;Finalizamos imprimendo la certeza obtenida, si es 0 o negativa decimos que no le gustará
(defrule fin_certeza
(declare (salience -2))
?m <- (modulo calcular_certeza_gustar_a_ninios)
(FactorCerteza ninios si ?f1)
=>
(if (> ?f1 0)
then
(printout t crlf " A los niños le gustará esta receta con una certeza de " (* ?f1 100) " %."crlf)
else
(printout t crlf " A los niños seguramente no le gustará esta receta. "crlf)
)
(printout t "Los motivos son: " crlf)
(do-for-all-facts ((?j justificacion_certeza)) (eq ?j:problema ninios)
